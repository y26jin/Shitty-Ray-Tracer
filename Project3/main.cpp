...
int main()
{
    ...
    int nx{ 200 };
    int ny{ 100 };
    CpuMode::ShittyPresent<int> p(nx, ny);
    CpuMode::FrameBuffer<int> fb;
    
    // draw every pixel onto fb
    CpuMode::vec3<double> lower_left_corner(-2.0, -1.0, -1.0);
    CpuMode::vec3<double> horizontal(4.0, 0.0, 0.0);
    CpuMode::vec3<double> vertical(0.0, 2.0, 0.0);
    CpuMode::vec3<double> origin(0.0, 0.0, 0.0);
    for (int j=ny-1;j>=0;--j)
    {
        for (int i = 0; i < nx; ++i)
        {
            ...
            CpuMode::ray<double> r(origin, lower_left_corner + u * horizontal + v * vertical);
            // draw a red ball with blue background
            CpuMode::vec3<double> col = CpuMode::color<double>(r);
